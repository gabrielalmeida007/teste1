...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:FRIBOI\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid==5515998576105:+55 1599857-6105\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','*'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ Espere...(caso nao consiga tente novamente mas nao flode)',
					success: '️❬ ✔ ❭ Sucesso 🖤',
				...
				only: {
					...
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado. Registre-se\n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar friboi|17\n\n──「 Friboi Bot 」──`,
			...
			const ownerNumber = ["5515998576105@s.whatsapp.net"] // replace this with your number
			...
			const premium = ["5515998576105@s.whatsapp.net"]
    ...
    break
		...
		case 'bpfont':
			...
			break 
				...
				case 'del':
				case 'd':  
					if (!isPremium) return reply('Você não é um Membro Premium' ,text, { quoted: mek })
					if (!isGroup)return reply(mess.only.group)
					if (!isGroupAdmins)return reply(mess.only.admin)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO Friboi Bot 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Friboi Domina vida
        ...
        case 'promover':
			...
			try {
				const s = await axios.get(`http://simsumi.herokuapp.com/api?text=${body.slice(6)}&lang=pt`)
			...
			await kill.reply(from, 'Frase adicionada.', id)
			break  
                case 'detector' :
            ...
            break
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[FRIBOI]','red'), 'cringe ', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
